 ...
 * este servicio realiza una conversión previa a String antes de almacenar los
...
public class ServicioLogOperaciones {
	...
	/**
	 * Formato estándar para almacenar la fecha y hora como texto, compatible con
	 * DB4O.
	 */
	private static final DateTimeFormatter FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

	public List<LogOperacionDB4O> findAll() {

		ObjectContainer db = null;
		List<LogOperacionDB4O> lista = new ArrayList<>();

		try {
			db = db40Manager.open();

			// Consulta nativa DB4O basicamente devuelve todos los objetos LogOperacionDB4O
			List<LogOperacionDB4O> resultados = db.query(LogOperacionDB4O.class);

			lista.addAll(resultados);

		} catch (Exception e) {
			System.err.println("Error leyendo logs DB4O: " + e.getMessage());
		} finally {
	 ...
	 *
	 * Este método: - Crea un objeto LogOperacion con los datos originales -
	 * Convierte los campos no soportados por DB4O (fecha, enum) a String - Crea un
	 * objeto LogOperacionDB4O compatible con DB4O - Lo almacena en la base de datos
	 * embebida - Gestiona commit/rollback y cierre seguro de la conexión
	 *
